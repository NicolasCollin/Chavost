...
DATA_PATH = Path(__file__).parents[2] / "data" / "base_cryptee.csv"  # ../.. /data/base_cryptee.csv
...
def load_data(path: Path) -> pd.DataFrame:
    ...
    for kwargs in (
        ...
        try:
            ...
            df = None
    if df is None:
        raise RuntimeError("Impossible de lire le CSV. Vérifie le séparateur et les formats numériques.")
    # Normalisation noms colonnes
    df.columns = [c.strip().lower() for c in df.columns]
    # Cast colonnes attendues
    expected = ["annee", "type_produit", "nom_produit", "quantite", "prix", "vecteur_id"]
...
with st.sidebar:
    ...
    prix_is_unit = st.toggle("Le champ 'prix' est un prix unitaire", value=False,
...
with c2:
    st.metric("Produits distincts", f"{fdf['nom_produit'].nunique():,}".replace(",", " "))
with c3:
    st.metric("Quantité totale", f"{int(np.nansum(fdf['quantite'])):,}".replace(",", " "))
with c4:
...
tab_overview, tab_evol, tab_types, tab_products, tab_prices, tab_table = st.tabs([
    "Vue d'ensemble", "Évolution", "Types & canaux", "Produits", "Analyse des prix", "Table"
])
...
with tab_overview:
    ...
    # CA par année (barres)
    by_year = fdf.groupby("annee", dropna=True, as_index=False).agg(ca=("ca", "sum"), qte=("quantite", "sum"))
    if not by_year.empty:
        fig = px.bar(by_year.sort_values("annee"), x="annee", y="ca", title="Chiffre d'affaires par année")
        fig.update_layout(xaxis_title="Année", yaxis_title="CA")
        c1.plotly_chart(fig, use_container_width=True)
    # Répartition par type (camembert)
    by_type = fdf.groupby("type_produit", as_index=False)["ca"].sum().sort_values("ca", ascending=False)
    if not by_type.empty:
        pie = px.pie(by_type, names="type_produit", values="ca", title="Répartition CA par type")
...
with tab_evol:
    ...
    # Série temporelle CA
    if not by_year.empty:
        line = px.line(by_year.sort_values("annee"), x="annee", y="ca", markers=True, title="Évolution du CA")
        c1.plotly_chart(line, use_container_width=True)
        # Tendance quantités
        line_q = px.line(by_year.sort_values("annee"), x="annee", y="qte", markers=True, title="Évolution des quantités")
    ...
    if not bt.empty:
        fig = px.bar(bt, x="annee", y="ca", color="type_produit", barmode="group",
...
with tab_types:
    ...
    # CA par type
    if not by_type.empty:
        bar_t = px.bar(by_type, x="type_produit", y="ca", title="CA par type de produit")
        bar_t.update_layout(xaxis_title="Type", yaxis_title="CA")
        c1.plotly_chart(bar_t, use_container_width=True)
    # CA par vecteur
    by_vec = fdf.groupby("vecteur_id", as_index=False)["ca"].sum().sort_values("ca", ascending=False)
    if not by_vec.empty:
...
with tab_products:
    ...
    c1, c2 = st.columns(2)
    if not top_ca.empty:
        bar_top = px.bar(top_ca, x="nom_produit", y="ca", title=f"Top {top_n} produits (CA)")
        ...
        bar_q = px.bar(top_ca, x="nom_produit", y="quantite", title=f"Top {top_n} produits (quantités)")
    ...
    prods = top_ca["nom_produit"].tolist() or sorted(fdf["nom_produit"].unique().tolist())
    sel_prod = st.selectbox("Produit (détail)", prods)
    if sel_prod:
        p = (fdf.loc[fdf["nom_produit"] == sel_prod]
                 .groupby("annee", as_index=False)
                 .agg(ca=("ca", "sum"), quantite=("quantite", "sum")))
        if not p.empty:
            fig1 = px.bar(p, x="annee", y="ca", title=f"CA par année — {sel_prod}")
            fig2 = px.line(p, x="annee", y="quantite", markers=True, title=f"Quantités par année — {sel_prod}")
...
with tab_prices:
    ...
    if fdf["type_produit"].nunique() > 0:
        box = px.box(fdf, x="type_produit", y="prix", points="outliers", title="Prix par type de produit")
        c2.plotly_chart(box, use_container_width=True)
    # Scatter prix vs quantite
    if fdf["quantite"].notna().sum() > 0:
        sc = px.scatter(fdf, x="quantite", y="prix", color="type_produit",
                        title="Prix vs Quantité", hover_data=["nom_produit", "annee", "vecteur_id"])
...
with tab_table:
    ...
    st.download_button("Télécharger (CSV)", csv_buf.getvalue(), file_name="export_filtre.csv", mime="text/csv")

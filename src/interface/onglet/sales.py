...
def get_and_prepare_data():
    ...
    if "code_article" in df_master.columns and "code_article" in df_arts.columns:
        ...
        df_master = pd.merge(
            df_master,
            df_arts[
                ["code_article", "libell_", "unit_"]
            ],  # libell_ comes from "Libellé"
            on="code_article",
            how="left",
        )
    ...
    if "n_document" in df_master.columns and "n_document" in df_histo.columns:
        ...
        df_master = pd.merge(df_master, histo_subset, on="n_document", how="left")
...
def render_sales_page():
    ...
    with st.sidebar:
        ...
        if "date" in df.columns:
            ...
            if isinstance(rng, tuple) and len(rng) == 2:
                mask = (df["date"] >= pd.to_datetime(rng[0])) & (
                    df["date"] <= pd.to_datetime(rng[1])
                )
    ...
    col_val = (
        "valeur_du_mouvement" if "valeur_du_mouvement" in df_filtered.columns else None
    )
    ...
    with tab1:
        ...
        if col_client and col_val:
            top_cli = (
                df_filtered.groupby(col_client)[col_val]
                .sum()
                .nlargest(10)
    ...
    with tab2:
        ...
        if col_prod_name in df_filtered.columns and col_qty and col_val:
            df_prod = (
                df_filtered.groupby(col_prod_name)[[col_qty, col_val]]
                .sum()
                .reset_index()
            )
    ...
    with tab3:
        if "date" in df_filtered.columns and col_val:
            ...
            fig_time = px.line(
                df_time, x="date", y=col_val, markers=True, title="Évolution Temporelle"
            )

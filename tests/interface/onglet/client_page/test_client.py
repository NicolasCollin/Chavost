...
class TestClientTool:
    ...
    def test_selection_declenche_les_appels_et_affichages(self, monkeypatch) -> None:
        ...
        df_com = pd.DataFrame({"nom_du_client": ["A"], "n_document": [1], "net_payer": [10.0]})
        df_sto = pd.DataFrame({"n_document": [1], "article": ["X"], "quantit_": [-1], "mois_annee": ["2025-01"]})
        ...
        monkeypatch.setattr(client_mod, "prepare_client_filters", lambda *_a, **_k: (df_clients.head(1), df_com, df_sto))
        ...
        monkeypatch.setattr(client_mod, "query_suivi_temp", lambda _con: pd.DataFrame({"nom": ["A"], "quantite": [1], "mois_annee": ["2025-01"]}))
        ...
        monkeypatch.setattr(client_mod, "render_kpis", lambda *_a, **_k: render_calls.append(None))
